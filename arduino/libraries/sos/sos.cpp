...
Sos::Sos(l2addr *mac_addr, int eth_type, uint8_t uuid) 
: _ttl(SOS_DEFAULT_TTL), _status(SL_COLDSTART), _uuid(uuid) {
	_eth = new EthernetRaw();
	//_master_addr = new l2addr_eth("ff:ff:ff:ff:ff");
	_master_addr = NULL; //new l2addr_eth("ff:ff:ff:ff:ff");
		...
		case SL_COLDSTART :
			...
			do {
				...
				if(_coap->coap_available()) {
					...
					if(strncmp((const char*)payload, "POST /.well-known/sos?hello=", 
