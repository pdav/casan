...
tbl_dtls <- read.table(file="screenlog-dtls.0.dur")
val_dtls <- tbl_dtls[1]

tbl_dtls_simplified <- read.table(file="screenlog-simplified.0.dur")
...
#boxplot(data)
boxplot(data, las = 2
        , ylab = "durée aller/retour d'un paquet (ms)", xlab = "Versions de DTLS"
        , names = c("DTLS", "DTLS\nsimplifié")
        , col=c("magenta", "cyan")
        )
