...
tbl_dtls <- read.table(file="dtls.0.enc")
val_dtls <- tbl_dtls[1]

tbl_dtls_simplified <- read.table(file="dtls-simplified.0.enc")
...
#boxplot(data)
boxplot(data, las = 2
        , ylab = "durée chiffrement d'un paquet (µs)", xlab = "Versions de DTLS"
        , names = c("DTLS", "DTLS\nsimplifié")
        , col=c("magenta", "cyan")
        )
