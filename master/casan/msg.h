...
class msg
{
	...
	// CASAN_HELLO is never used
	typedef enum casantype { CASAN_NONE=0,
		    CASAN_DISCOVER, CASAN_ASSOC_REQUEST,
		    CASAN_ASSOC_ANSWER, CASAN_HELLO, CASAN_UNKNOWN}
		casantype_t ;
	...
	// control messages
	casantype_t casan_type (void) ;
	bool is_casan_discover (slaveid_t &sid, int &mtu) ;// CASAN control message
	bool is_casan_associate (void) ;		// CASAN control message
	void add_path_ctl (void) ;
	void mk_ctl_hello (long int hid) ;		// options for ctl mssage
	void mk_ctl_assoc (casantimer_t ttl, int mtu) ;	// options for ctl mssage
	...
	casantype_t casantype_ = CASAN_UNKNOWN ;
	...
	bool is_casan_ctl_msg (void) ;
	casantype_t casan_type (bool checkreqrep) ;
} ;
