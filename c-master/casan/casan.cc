/**
 * @file casan.cc
 * @brief CASAN engine implementation
 */
...
/**
 * @brief CASAN engine destructor
 *
 * This destructor removes all internal lists used by the CASAN engine.
 *
...
/**
 * @brief Start the CASAN engine
 *
 * This function starts the CASAN engine, i.e. the sender thread
 * with the casan::sender_thread function.
 * Note that each receiver thread will be started with an appropriate
 * call to casan::start_net.
 */

void casan::init (void)
{
    std::srand (std::time (0)) ;

    if (tsender_ == NULL)
    {
	tsender_ = new std::thread (&casan::sender_thread, this) ;
    }
}

/**
 * @brief Dumps the CASAN engine data on the given stream
 *
 * This method is used for debugging purpose.
 *
 * @param os output stream
 * @param se CASAN engine object
 */

std::ostream& operator<< (std::ostream &os, const casan &se)
{
    os << "Receivers: " ;
...
/**
 * @brief Dumps CASAN engine configuration
 *
 * Returns a string with the CASAN engine configuration (global
...
std::string casan::html_debug (void)
{
 ...
 * managed by the CASAN engine, and asks the sender thread to
 ...
 *
 * This method adds a new slave to the CASAN engine. In detail,
 * this means:
...
slave *casan::find_slave (slaveid_t sid)
{
    slave *r ;
...
void casan::add_request (msgptr_t m)
{
    std::unique_lock <std::mutex> lk (mtx_) ;
...
void casan::sender_thread (void)
{
	...
	for (auto &r : rlist_)
	{
		...
		r->thr = new std::thread (&casan::receiver_thread, this, r) ;
	    }
...
void casan::receiver_thread (receiver *r)
{
    for (;;)
    {
	...
	/*
	 * Check CASAN control messages first
	 */

	if (m->casan_type () != msg::CASAN_NONE)
	{
	    m->peer ()->process_casan (this, m) ;
	    continue ;
	}
...
bool casan::find_peer (msgptr_t m, l2addr *a, receiver &r)
{
    bool found ;
	...
	if (!found)
	{
	    ...
	    if (m->is_casan_discover (sid, mtu))
	    {
		for (auto &s : slist_)
		{
		    if (sid == s.slaveid ())
...
msgptr_t casan::correlate (msgptr_t m)
{
    msg::msgtype mt ;
    msgptr_t orgmsg ;
...
void casan::clean_deduplist (receiver &r)
{
...
msgptr_t casan::deduplicate (receiver &r, msgptr_t m)
{
    msg::msgtype mt ;
    msgptr_t orgmsg ;
